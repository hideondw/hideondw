...
void procesar(unsigned char *mata, unsigned char *matb, unsigned char *matc, int n, int alig);

float GetTime();

void main() {
  float t1, t2;
  __attribute__((aligned(16))) unsigned char mat1[N][N];
  int i;
  char tonto;
  __attribute__((aligned(16))) unsigned char mat2[N][N];
  char tonto2;
  __attribute__((aligned(16))) unsigned char mat3[N][N];
  ...
  for (i=0;i<M;i++)  
    procesar(&mat1[0][0],&mat2[0][0],&mat3[0][0],N, 1);
